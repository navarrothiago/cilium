...
func (s *XDSServer) getListenerConf(name string, kind policy.L7ParserType, port uint16, isIngress bool, mayUseOriginalSourceAddr bool) *envoy_config_listener.Listener {
	...
	if kind == policy.ParserTypeHTTP {
		...
		// Add a filter that matches HTTPS and uses an HTTP egress cluster
...
func (s *XDSServer) UpdateSVIDs(id identity.NumericIdentity, svids []*workload.X509SVID) {
...
func convertSVIDs(svids []*workload.X509SVID) ([]*cilium.X509SVID, error) {
